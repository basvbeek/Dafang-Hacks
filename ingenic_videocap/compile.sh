...
rm CMakeCache.txt
rm -r CMakeFiles
cmake -DCMAKE_TOOLCHAIN_FILE="./dafang.toolchain"  -DCMAKE_INSTALL_PREFIX=${INSTALL} --debug-output && make VERBOSE=1 -j4 install
#-DCMAKE_LIBRARY_PATH=$ROOTPATH/toolchain/mips-linux-gnu/libc/usr/lib
err=$?
if [ $err != 0 ]; then
    exit $err
else
